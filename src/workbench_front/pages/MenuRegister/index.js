          ...
          if (this.state.isedit) {
            return (
              <EditableCell
                ...
                onChange={this.onCellChange(record.pk_menu, "menucode")}
                onCheck={this.onCellCheck(record.pk_menu, "menucode")}
              />
            );
          } else {
            return <div>{text}</div>;
          ...
          if (this.state.isedit) {
            return (
              <EditableCell
                hasError={this.state.iserror}
                value={text}
                onChange={this.onCellChange(record.pk_menu, "menuname")}
                onCheck={this.onCellCheck(record.pk_menu, "menuname")}
              />
            );
          } else {
            return <div>{text}</div>;
          ...
          if (this.state.isedit) {
            return (
              <EditableCell
                hasError={this.state.iserror}
                value={text}
                onChange={this.onCellChange(record.pk_menu, "menudesc")}
                onCheck={this.onCellCheck(record.pk_menu, "menudesc")}
              />
            );
          } else {
            return <div>{text}</div>;
  ...
  handleBtnClick = key => {
    switch (key) {
      case "save":
        ...
        let modifyData = this.state.listData.filter(item => item.isModify);
        Ajax({
          url: `/nccloud/platform/appregister/editappmenu.do`,
          data: modifyData,
          info: {
