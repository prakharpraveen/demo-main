...
class PageTable extends Component {
  ...
  jumpPage = record => {
    openPage(`/Zone`, false, { n: "页面模板设置", templetid: record.pk_page_templet });
  };
  /**
   * 设置默认模板
   */
        ...
        success: ({ data }) => {
          if (data.success && data.data) {
            if (record.pk_btn) {
              ...
              _.remove(
                newData,
                item => record.pk_page_templet === item.pk_page_templet
              );
            }
            this.setNewData(newData);
            this.cacheData = _.cloneDeep(newData);
            ...
            if (listData.pk_btn || listData.pk_page_templet) {
              newData.map((item, index) => {
                if (
                  ...
                  listData.pk_page_templet === item.pk_page_templet
                ) {
  ...
  add() {
    ...
    if (activeKey === "1") {
      ...
      openPage(
        `/Zone`,
        false,
        { n:"页面模板设置",pid: id, pcode: code, appcode: parentcode }
      );
    }
  }
  getNewData() {
