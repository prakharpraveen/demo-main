		...
		return data.map((e, i) => {
			...
			return (
				<TreeNode
					...
					treeNodeData={e}
					disableCheckbox={onlyLeafCanSelect === true && e.isleaf === false}
				>
...
class Ref extends PopRefer { // 继承PopRefer类
    ...
    onTreeTypeChange = (value) =>{
        this.setState({
            ...
            let param = {...this.getParam(),  treetype: this.state.treetype, disabledDataShow: false};
            this.loadTreeData(param).then((data) => {
                this.setTreeData('treeData', {refname:value === 'sobtablename' ? '账簿类型': '主账簿',refpk:'root', treeid: 'root'}, data);
            });
        });
    };
    getParam = (param = {}) => {
    ...
    onTreeNodeSelectWapper(selectedKeys, { selected, selectedNodes, node, event }, ...rest){
        if(this.state.treetype == 'sobtablename' && node.props.treeNodeData.pid == 'root')
            return;
    ...
    setTreeData = (target, parentNode, data, cb) => {
		...
		data.rows.forEach((e) => {
			e._display = this.props.treeConfig.code.map((item) => e[item] || e.values[item].value).join(' ');
			e.pid = e.pid || rootNode.refpk;
		});

		let newpks = data.rows.map((e) => e.refpk);

		this.state[target] = this.state[target].filter((e) => !newpks.includes(e.refpk)).concat(data.rows);
